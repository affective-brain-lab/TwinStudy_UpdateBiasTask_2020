...
twinModel.summary <- function(x) {
  ...
  #The 'parameters-penalty' version takes the likelihood function and adds 2 times the number of 
  #parameters, while the 'df-penalty' takes the likelihood function and subtracts the model degrees of 
  #freedom. As model degrees of freedom is just data degrees of freedom minus the number of paramaters, 
  #these two versions will always differ by 2 * data degrees of freedom. There is a similar relationship 
  #for BIC. As long as you're making comparisons on the same dataset, it doesn't matter which penalty 
  #you use as long as you're consistent. (source: http://openmx.psyc.virginia.edu/thread/1359)
  summs <- summary(x)
